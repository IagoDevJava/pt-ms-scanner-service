...
- До 200 одновременных HTTP-соединений (настройка Tomcat)
- Пропускная способность: ~5000 быстрых поисков в секунду на кэше,
  ~500 сложных сканирований в секунду (зависит от ms-complex-scan)
- Задержка быстрого поиска: < 10ms при попадании в кэш, < 50ms при запросе к Library Service
